-- Migration: Fix RPC enum casting and campaign_type
-- Timestamp: 20260118195500

-- 1. Revert campaign_type to TEXT to support 'promote' and other dynamic types
ALTER TABLE public.campaigns ALTER COLUMN campaign_type TYPE TEXT;

-- 2. Update the RPC function to explicitly cast status to campaign_status_enum
--    and handle the new TEXT campaign_type (implicit cast)
...
BEGIN
    ...
    IF v_campaign_id IS NOT NULL THEN
        ...
        SET
            ...
            status = (campaign_data->>'status')::campaign_status_enum, -- Explicit cast to ENUM
            campaign_type = campaign_data->>'campaign_type', -- No cast needed (TEXT)
    ...
    ELSE
        ...
        VALUES (
            ...
            (campaign_data->>'status')::campaign_status_enum, -- Explicit cast to ENUM
            campaign_data->>'campaign_type', -- No cast needed (TEXT)
    ...
    IF posts_data IS NOT NULL AND jsonb_array_length(posts_data) > 0 THEN
        ...
        SELECT
            ...
            p->>'status' -- scheduled_posts.status is VARCHAR, likely doesn't need enum cast
