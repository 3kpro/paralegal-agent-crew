...
export const errors = {
  ...
  forbidden: (message?: string) => new AppError(403, message || 'Forbidden - Insufficient permissions'),
  notFound: (resource?: string) => new AppError(404, resource ? `${resource} not found` : 'Not found'),
  badRequest: (message: string, details?: any) => new AppError(400, message, details),
  conflict: (message: string) => new AppError(409, message),
  tooManyRequests: () => new AppError(429, 'Too many requests - Please try again later'),
