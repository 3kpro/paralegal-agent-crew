...
export default function PublishButton({
  ...
  async function loadSocialAccounts() {
    ...
    try {
      ...
      if (result.success) {
        const activeAccounts = result.accounts.filter((acc: SocialAccount) => acc.is_active)
  ...
  const buttonVariants = {
    ...
    secondary: 'bg-tron-grid hover:bg-tron-dark border border-tron-cyan/30 text-tron-text'
  }
            ...
            >
              ...
              {loading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <h4 className="text-lg font-semibold text-tron-text mb-2">No Social Accounts Connected</h4>
                ...
                <div className="space-y-6">
                  {/* Content Preview */}
                  <div>
                    <label className="block text-sm font-medium text-tron-text mb-2">Content to Publish</label>
                    <div className="bg-tron-dark/50 border border-tron-cyan/20 rounded-lg p-4 max-h-32 overflow-y-auto">
                      <p className="text-sm text-tron-text whitespace-pre-wrap">{content}</p>
                    </div>
                    <p className="text-xs text-tron-text-muted mt-1">{content.length} characters</p>
                  </div>

                  {/* Social Account Selection */}
                  <div>
                    <label className="block text-sm font-medium text-tron-text mb-3">
                      Select Social Accounts ({selectedAccounts.length} selected)
                    </label>
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <span className="font-medium text-tron-text">{account.account_name}</span>
                                {account.is_verified && (
                  ...
                  <div>
                    <label className="flex items-center gap-2 mb-3">
                      <input
                        ...
                        className="w-4 h-4 text-tron-cyan bg-tron-dark border-tron-cyan/30 rounded focus:ring-tron-cyan"
                      />
                      <span className="text-sm font-medium text-tron-text">Schedule for later</span>
                    </label>
                    ...
                    {isScheduled && (
                      ...
                      >
                        <div>
                          <label className="block text-sm text-tron-text-muted mb-1" htmlFor="scheduled-date">Date</label>
                          <input
                            id="scheduled-date"
                            type="date"
                            value={scheduledDate}
                        ...
                        <div>
                          <label className="block text-sm text-tron-text-muted mb-1" htmlFor="scheduled-time">Time</label>
                          <input
                            id="scheduled-time"
                            type="time"
                            value={scheduledTime}
