...
export function PublishSettings({ settings, onChange, onBack, campaignData }: PublishSettingsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {(error || showError) && (
        ...
        >
          <span className="block sm:inline">{error || 'Please select at least one platform'}</span>
        </div>
      )}
      ...
      <div className="flex justify-between pt-6">
        ...
        >
          {publishing ? (
            <>
              <Loader2 data-testid="loading-spinner" className="w-4 h-4 animate-spin mr-2" />
              Publishing...
            </>
          ) : (
            'Publish Now'
