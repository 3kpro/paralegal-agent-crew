...
export const EnhancedTwitterDemo: React.FC<DemoModalProps> = ({ isOpen, onClose }) => {
  ...
  const [style, setStyle] = useState<'professional' | 'casual' | 'educational'>('professional')
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const pollInterval = setInterval(async () => {
          try {
            const statusResponse = await fetch(`/api/twitter-post?trackingId=${data.trackingId}`)
        ...
        setTimeout(() => {
          clearInterval(pollInterval)
          if (isGenerating) {
            setError('Posting timed out. Please check your Twitter account manually.')
            setIsGenerating(false)
          }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">🚀 Content Cascade AI - Enhanced Twitter Integration</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">✨ Enhanced Twitter Posting Demo</h3>
            ...
            <form onSubmit={handleGenerateAndPost} className="space-y-4">
              ...
              <div className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <label htmlFor="includeTrends" className="text-sm text-gray-700">
          ...
          {result && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
              <h4 className="font-semibold text-blue-800 mb-4">📝 Processing Started:</h4>
              <div className="space-y-2">
                <p><strong>Tracking ID:</strong> {result.trackingId}</p>
                <p><strong>Status:</strong> {isGenerating ? 'Processing...' : 'Complete'}</p>
                <p><strong>Estimated Time:</strong> {result.estimatedTime}</p>
                {result.metadata && (
                  <>
                    <p><strong>Content Length:</strong> {result.metadata.contentLength} characters</p>
                    <p><strong>Estimated Thread Length:</strong> {result.metadata.estimatedThreadLength} tweets</p>
                    <p><strong>Includes Trends:</strong> {result.metadata.includesTrends ? 'Yes' : 'No'}</p>
                    <p><strong>Posting Account:</strong> {result.metadata.postingAccount}</p>
                  </>
                )}
              </div>
            </div>
          ...
          {status && status.status === 'completed' && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
              <h4 className="font-semibold text-green-800 mb-4">🎉 Successfully Posted to Twitter!</h4>
              {status.result && (
          ...
          <div className="grid md:grid-cols-3 gap-4 text-center">
            <div className="p-4 bg-blue-50 rounded-lg">
              <h4 className="font-semibold text-blue-800 mb-2">📱 Twitter Posting</h4>
