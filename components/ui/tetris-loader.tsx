...
// Tetris pieces - Site Theme Colors
const TETRIS_PIECES = [
  // I-piece - Cyan (Tron)
  { shape: [[1, 1, 1, 1]], color: 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.6)]' },
  // O-piece - Coral (Brand)
  { shape: [[1, 1], [1, 1]], color: 'bg-coral-500 shadow-[0_0_8px_rgba(255,126,95,0.6)]' },
  // T-piece - Purple
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.6)]' },
  // L-piece - Orange
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.6)]' },
  // S-piece - Green
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.6)]' },
  // Z-piece - Red/Pink (Magenta)
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-pink-500 shadow-[0_0_8px_rgba(236,72,153,0.6)]' },
  // J-piece - Blue
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]' },
]
...
export default function TetrisLoading({ 
  ...
  // Size configurations - SHORTENED for compactness
  const sizeConfig = {
    sm: { cellSize: 'w-1.5 h-1.5', gridWidth: 8, gridHeight: 12, padding: 'p-0.5' },
    md: { cellSize: 'w-2 h-2', gridWidth: 10, gridHeight: 15, padding: 'p-1' }, // Reduced height from 20 to 15, cellSize from w-3 to w-2
    lg: { cellSize: 'w-2.5 h-2.5', gridWidth: 10, gridHeight: 18, padding: 'p-1.5' }
  }
          ...
          <div
            ...
            className={`${config.cellSize} border border-white/5 transition-all duration-100 ${
              cell.filled 
                ...
                : 'bg-white/5 scale-95'
            } ${isClearing && rowIndex < 4 ? 'animate-pulse bg-white/20' : ''}`}
          />
        ))}
      </div>
    ))
  ...
  return (
    <div>
      <div className="mb-4 flex justify-center">
        <div className={`border-2 border-tron-grid bg-tron-dark/50 backdrop-blur-md rounded-xl shadow-xl ${config.padding} transition-colors`}>
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-gray-400 font-medium text-sm animate-pulse">{loadingText}</p>
        </div>
      )}
    </div>
  )
