...
export const DotLoader = ({
    ...
    return (
        <div {...props} ref={gridRef} className={cn("grid w-fit grid-cols-7 gap-0.5", className)} data-testid="dot-loader">
            {Array.from({ length: 49 }).map((_, i) => (
                <div key={i} data-dot-index={i} className={cn("h-1.5 w-1.5 rounded-sm", dotClassName)} />
            ))}
        </div>
    );
};