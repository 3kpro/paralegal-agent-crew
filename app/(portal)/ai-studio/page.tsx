...
export default function AIStudioPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "generate" | "templates" | "variations" | "analytics"
  >("generate");
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-tron-text flex items-center gap-3">
                <Brain className="w-8 h-8 text-tron-cyan" />
                AI Studio
              </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {providers.map((provider) => {
              ...
              return (
                <div
                  key={provider.id}
                  className="flex items-center justify-between bg-tron-dark/50 rounded-lg p-3"
                >
                  <div className="flex items-center gap-3">
                    {Icon && <Icon className="w-5 h-5 text-tron-cyan" />}
                    <div>
                  ...
                  <div
                    className={`w-2 h-2 rounded-full ${provider.enabled ? "bg-green-400" : "bg-red-400"}`}
                  ></div>
                </div>
              );
            })}
      ...
      <div className="lg:col-span-1 space-y-6">
        ...
        <div className="bg-tron-grid border border-tron-cyan/30 rounded-lg p-6">
          ...
          <div className="space-y-2">
            {templates.slice(0, 5).map((template: PromptTemplate) => (
              <button
                key={template.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-tron-text-muted">
                    {template.average_quality.toFixed(1)}
                  </span>
          ...
          <div className="bg-tron-grid border border-tron-cyan/30 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {selectedTemplate.variables.map((variable: string) => (
                <div key={variable}>
                  <label className="text-sm font-medium text-tron-text mb-1 block">
                    {variable}
                  </label>
                  <input
                    type="text"
        ...
        <div className="bg-tron-grid border border-tron-cyan/30 rounded-lg p-6">
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto">
            {generationResults.length === 0 ? (
              ...
              generationResults.map((result: GenerationRequest) => (
                <div
                  key={result.id}
...
function VariationsView() {
  return (
    <div className="text-center py-12">
      ...
      <p className="text-tron-text-muted">
        A/B testing and content optimization tools
      </p>
    </div>
  );
}
