...
export async function POST(request: NextRequest) {
  try {
    ...
    const { provider, model, prompt, session_id, max_tokens = 1000, temperature = 0.7 } = body
    ...
    try {
      ...
      if (provider === 'gemini') {
        ...
        content = await generateWithGemini(prompt, model, max_tokens, temperature)
        ...
        content = await generateWithLMStudio(prompt, model, max_tokens, temperature)
        tokens_used = estimateTokens(content)
      ...
      return NextResponse.json({
        success: false,
        error: generationError instanceof Error ? generationError.message : 'Content generation failed'
      }, { status: 500 })
    }
...
async function generateWithGemini(prompt: string, model: string, maxTokens: number, temperature: number): Promise<string> {
...
async function generateWithLMStudio(prompt: string, model: string, maxTokens: number, temperature: number): Promise<string> {
  // TODO: Implement actual LM Studio API call
  // For now, return a realistic mock response
  await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 1500)) // Simulate local processing
