...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!title || !content || !platform || !scheduled_at) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: title, content, platform, scheduled_at' },
        { status: 400 }
      )
    }

    // Validate platform
    const validPlatforms = ['twitter', 'linkedin', 'facebook', 'instagram', 'tiktok', 'reddit', 'youtube']
