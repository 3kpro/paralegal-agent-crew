...
export async function GET(request: NextRequest) {
  try {
    ...
    // Process queue items
    const processedQueue = queueItems?.map(item => ({
      ...item,
    ...
    const stats = {
      ...
      pending: processedQueue.filter(item => item.scheduled_post.status === 'scheduled').length,
      processing: processedQueue.filter(item => item.scheduled_post.status === 'publishing').length,
      failed: processedQueue.filter(item => item.scheduled_post.status === 'failed').length,
      completed: processedQueue.filter(item => item.scheduled_post.status === 'published').length
    }
